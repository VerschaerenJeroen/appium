...
Open `Cortana` and search for `Edit environment variables for your account`. In the menu click on bottom button called `Environment Variables` and this will open a pop-up. Here make sure under `System variables` there is a variable called `JAVA_HOME` and `ANDROID_HOME`, if not then we will have to add a new variable with the names previously mentioned. As for their value the `JAVA_HOME` should have the directory path to your local java `JDK Folder` and for `ANDROID_HOME` to your local android `SDK Folder`
